...
//para ejecutarlo basta con: ./x (y los parametros)
...
int main(void) { //funcion main donde se ejecutara tod0 el programa y se escribirá la mayor parte del codigo
    ...
    tline * line; //tipo dato que representa los datos extraidos de la linea de comandos
    ...
    while (fgets(buffer,1024,stdin)){ //bucle del programa
        line = tokenize(buffer); //tokenize del buffer (funcion proporcinada por el enunciado)
        ...
        if(line->ncommands == 0) {
            ...
            pipeArray = (int **)malloc((line->ncommands-1)*sizeof(int *)); //reservamos memoria para el array de pipes
            ...
            for (i=0; i<line->ncommands; i++) { //realizamos un bucle con el numero de comandos introducidos (>1)
                switch (pid = fork()) { //se hace el fork dentro del switch y es el pid con el que se van eligiendo los case
                    ...
                    case 0:   // Proceso Hijo (pid = 0)
                        if (i == 0) { //primer mandato
                            for (j=1;j<line->ncommands-1;j++) { //cierras todos los pipes que no vayas a utilizar
                                close(pipeArray[j][0]);
                                close(pipeArray[j][1]);
                            }
                            close(pipeArray[0][0]); //cerramos el extremo de lectura (del primer pipe) ya que vamos a escribir

                            if (line->redirect_input != NULL) { // < (el primer mandato solo podrá tener redirección de entrada)
                                fd = open(line->redirect_input,O_RDONLY); //abrimos el fichero
                                if (fd != -1) {
                                    dup2(fd, 0); //redirigimos la entrada
                                    close(fd); //cerramos el fd
                            ...
                            for (j=0; j<line->ncommands-2; j++) { //cerramos los pipes menos el ultimo que es el que usaremos
                                close(pipeArray[j][0]);
                                close(pipeArray[j][1]);
                            }
                            close(pipeArray[line->ncommands-2][1]); //cerramos el extremo de escritura

                            if (line->redirect_output != NULL) { // > (el ultimo mandato solo podrá tener redirección de salida y de error)
                                fd = creat(line->redirect_output, 0666); //creamos el fichero
                                if (fd!=-1) {                                   // permisos del mode 0666: R y W para user group y other
                                    dup2(fd, 1); //redirigimos la salida
                                    close(fd); //cerramos el fd
                            ...
                            for (aux=i+1; aux<line->ncommands-1; aux++) { //cerramos los pipes que no usaremos
                                close(pipeArray[aux][0]);
                                close(pipeArray[aux][1]);
                            }
